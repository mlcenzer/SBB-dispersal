...
data_T2$sym_dist<-abs(data_T2$latitude-25.49197) ##Doesn't need to be recalculated; this isn't actually a centering process
...
models <- models_init$ix[1:top]
probs <- models_init$x[1:top]
rbind(AICs, models, probs) # why does m5 show up if it doesn't exist? MLC: the null model (m0) is fifth in the order; it's always the last one in the list. In this model set, the null model is performing the best.
```
