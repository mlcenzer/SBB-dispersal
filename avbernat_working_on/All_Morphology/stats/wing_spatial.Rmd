...
```{r}
unique(d2[,c("population", "lat","long")])
```
...
LL <- LongLatToUTM(x = d2$long, y = d2$lat, zone = 31)
d2$X.utm <- LL$X
d2$Y.utm <- LL$Y
```
...
```{r}
LL <- LongLatToUTM(x = d3$long, y = d3$lat, zone = 31)
d3$X.utm <- LL$X
d3$Y.utm <- LL$Y
...
```{r}
LL <- LongLatToUTM(x = d4$long, y = d4$lat, zone = 31)
d4$X.utm <- LL$X
d4$Y.utm <- LL$Y
