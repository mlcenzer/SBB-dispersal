...
Keys.UTM = crop(Florida_ShapeFile.UTM, extent(439300.8+700000, 1194383, 12721244-1200000, 13431336-600000))
```
...
data(World)
Florida_ShapeFile.UTM <- spTransform(World, 
                           CRS("+proj=utm +zone=16 +south +ellps=WGS84 +datum=WGS84"))
World.ae <- st_transform(World, "+proj=merc +lon_0=-69 +k=1 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs")

```

```{r}
extent(Keys.UTM)[1] - extent(Keys.UTM)[2]
extent(Keys.UTM)[3] - extent(Keys.UTM)[4]
```

```{r}
extent(Loc)[1] - extent(Loc)[2]
extent(Loc)[3] - extent(Loc)[4]
```
