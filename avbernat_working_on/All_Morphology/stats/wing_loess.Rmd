...
data<-data.frame(R=df$wing2body, 
                 A=df$sex, 
                 B=df$pophost, 
                 C=(df$dates))
A = data$A
...
fit0 <- glm(R~1, family="gaussian")
fit1 <- glm(R~A, family="gaussian")
fit2 <- glm(R~bs(C,5), family="gaussian") # b-splin basis for polynomial splines
fit3 <- glm(R~B + bs(C,5), family="gaussian")
...
plot(x,y)
abline(fit1, col='red')
xx <- seq(min(x),max(x), length.out=36)
...
```{r}
d$index = 1:nrow(d)
loessMod20 <- loess(wing2body ~ index, data=d, span=0.20, degree=0) # 20% smoothing span
loessMod40 <- loess(wing2body ~ index, data=d, span=0.40, degree=0) # 40% smoothing span
loessMod60 <- loess(wing2body ~ index, data=d, span=0.60, degree=0) # 60% smoothing span
loessMod80 <- loess(wing2body ~ index, data=d, span=0.80, degree=0) # 80% smoothing span
