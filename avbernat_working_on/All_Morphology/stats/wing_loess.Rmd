...
p5 = p5 + annotate(geom="text", x=10, y=0.74, label=alpha, color="black", parse=TRUE) +
          annotate(geom="text", x=10, y=0.743, label=degree, color="black", parse=TRUE) +
          annotate(geom="text", x=6.2, y=0.718, label=pvalue, color="black", parse=TRUE)

p6 = p6 + annotate(geom="text", x=10, y=0.74, label=alpha, color="black", parse=TRUE) +
          annotate(geom="text", x=10, y=0.743, label=degree, color="black",parse=TRUE) +
          annotate(geom="text", x=6, y=0.718, label=pvalue, color="black", parse=TRUE)
...
```{r}
# extract the LOESS
ggplot_build(p5)$data[[2]] # alpha = 0.4 (smoothing parameter)
ggplot_build(p6)$data[[2]] # alpha = 0.4 (smoothing parameter); lambda = degree of the local polynomial 
...
```{r fig.width=4.7, fig.height=2.3}
p9 = ggplot() + 
  ggtitle("Host Plant") + xlab("Month") + ylab("Long-Wing Morph Frequency") +
  geom_vline(xintercept = xlab_dates, color="gainsboro") + 
  ...
  geom_smooth(data=df, method="loess", 
              mapping = aes(x = month_of_year, y = wing_morph_binom, colour=pophost, fill=pophost)) + 
  geom_point(data=df, mapping = aes(x = month_of_year, y = wing_morph_binom, colour=pophost))
...
p10 = ggplot() + 
  ggtitle("Sex") + xlab("Month") + ylab("Long-Wing Morph Frequency") +
  geom_vline(xintercept = xlab_dates, color="gainsboro") + 
  geom_smooth(data=df, method="lm", se=FALSE, linetype = "dashed", lwd=0.5,
              mapping = aes(x = month_of_year, y = wing_morph_binom), colour="black") +
  geom_smooth(data=df, method="loess", 
              mapping = aes(x = month_of_year, y = wing_morph_binom, colour=sex, fill=sex)) + 
  geom_point(data=df, mapping = aes(x = month_of_year, y = wing_morph_binom, colour=sex)) 
...
p10 = p10 + guides(fill = guide_legend(reverse = TRUE)) + theme_classic() +
  ...
  labs(fill = "Host Plant") + theme(legend.title = element_text(size = 14),
                                    legend.text = element_text(size = 13, face="italic")) +
  scale_color_manual(values=sex_colors_pts) + theme(legend.position = c(0.2, 0.95)) + theme(legend.title = element_blank()) +
  scale_x_continuous(breaks=xlab_months, labels= month_labs)
...
p11 = ggplot() + 
  ...
  #ylim(0.71, 0.75) +
  stat_smooth(data=last_4_years, mapping = aes(x = dates, y = wing_morph_binom), method = "lm", formula = y ~ x, se = FALSE,
               colour="blue") # linetype = "longdash",
...
# p11 = p11 + annotate(geom="text", x=unique(df$dates)[10], y=0.74, 
#                    label=alpha, color="black", parse=TRUE, size=6) +
#           annotate(geom="text", x=unique(df$dates)[10], y=0.748, 
#                    label=degree, color="black",parse=TRUE, size=6) +
#           annotate(geom="text", x=unique(df$dates)[15], y=0.718, 
#                    label=pvalue, color="black", parse=TRUE, size=6)
# # last 4 years
# mlinear = glm(wing_morph_binom ~ dates, data=wm_table[wm_table$date_b=="2016-2020",], family=gaussian)
# pvalue = paste0("italic(p)[glm]==",round(summary(mlinear)$coeff[[8]],2))
# 
# p11 = p11 + annotate(geom="text", x=unique(df$dates)[24], y=0.743, label=pvalue, color="blue", parse=TRUE, size=6)
