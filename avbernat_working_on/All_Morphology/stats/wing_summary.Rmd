...
```{r results = "hold"}
###MLC:  I'm seeing m88 as the top model coming out of this set; the only one of these showing up for me is m58
...
```{r}
M3 = glm(wing2body_c ~ sex_binom*pophost_binom + pophost_binom*months_since_start_c + month_of_year_c, data=data_long, family=gaussian) 
summary(M3) 
```
...
```{r}
###MLC: function to calculate 95% CI. 
CI_95 <- function(x){qnorm(0.975)*sd(x)/sqrt(length(x))}
CI_95_binom_upper <- function(y) {binom.confint(x=sum(y, na.rm=TRUE), n=length(y[!is.na(y)]), conf.level=0.95, methods='exact')$upper}
CI_95_binom_lower <- function(y) {binom.confint(x=sum(y, na.rm=TRUE), n=length(y[!is.na(y)]), conf.level=0.95, methods='exact')$lower}
...
# was substituted (see around lines 471 for the same situation)
...
fit2 = glm(wing_morph_binom ~ month_of_year + sex_binom * pophost_binom + 
