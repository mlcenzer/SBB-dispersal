...
[Flight Case ~ Mass Diff ](#mass_diff)

[Flight Case ~ Mass Per ](#mass_per)
...
calculate_P2 = function(model) {
  ...
  colnames(model_table) <- c("(Intercept)","coeff mass_per", "coeff sex_c","DF", 
                            "m% SE", "sex SE", "m% z", "sex z", 
                            "ML wald","sex wald","m% P > |z|", "sex P > |z|")
  cat("\n", "AIC: ", s$AIC, "\n", "m% = mass_per", "\n") 
  print(model_table)
  return(model_table)
}
...
model_table = calculate_P2(delta_mass_model)
```

```{r echo=FALSE}
prediction_equations2 = function(tb) {
  # -1 / 1 | Flew in T1, not T2
  I = (tb[1,1] - tb[2,1])
  M = (tb[1,2] - tb[2,2])
  S = (tb[1,3] - tb[2,3])
  EQ1 = paste0("log(pi_-1 / pi_1) = ", round(I, 2), " + ", round(M,2), " Mass Percent Change", " + ", round(S, 2), " Sex ", "     Flew in T1, rather than T2" )
  
  # 2 / -1 | Flew in both, not T1
  I = (tb[3,1] - tb[1,1])
  M = (tb[3,2] - tb[1,2])
  S = (tb[3,3] - tb[1,3])
  EQ2 = paste0("log(pi_2 / pi_-1) = ", round(I, 2), " + ", round(M,2), " Mass Percent Change", " + ", round(S, 2), " Sex ", "     Flew in both, rather than T1" )
  
  # 2 / 1 | Flew in both, not T2
  I = (tb[3,1] - tb[2,1])
  M = (tb[3,2] - tb[2,2])
  S = (tb[3,3] - tb[2,3])
  EQ3 = paste0("log(pi_2 / pi_1) = ", round(I, 2), " + ", round(M,2),  " Mass Percent Change", " + ", round(S, 2), " Sex ", "     Flew in both, rather than T2")
...
```{r}
prediction_equations2(model_table)
```

```{r echo=FALSE}
eq1 = "Flew in T1 only rather than T2 only = 16.5 + 98.29 Mass % + 15.49 Sex"
eq2 = "Flew in both rather than T1 only = 1.25 - 50.49 Mass % - 0.1 Sex"
eq3 = "Flew in both rather than T2 only = 17.67 + 47.8 Mass % + 15.39 Sex"
...
- If F and loose mass then most likely to fly in T2 only rather than none. (but none did that)

? How to properly interpret these odds?
...
colnames(table) <- c("Intercept", "egg_diff","DF", "Std. Err.", "z", "wald", "P > |z|")
cat("\n")
cat("AIC: ", s$AIC, "\n")
table
```
...
# summary table (similar to lm, glm, or lmer function)
model_table <- cbind(s$coefficients, c(s$edf, s$edf), s$standard.errors[2], z[2], wald[2], p[2])
colnames(model_table) <- c("Intercept","egg_diff", "mass_diff", "DF", "Std. Err.", "z", "wald", "P > |z|")
cat("\n")
cat("AIC: ", s$AIC, "\n")
model_table
```
...
plot(df$mass_diff[egg_T1rows], pp[egg_T1rows,1], ylim=c(0,1), xlim=c(-0.045,0.06), col="red", type="l", lty=1,
     main="Females Only",
     #type="b", cex=0.45, pch=16, 
...
points(df$mass_diff[egg_T1rows], pp[egg_T1rows,3], col="maroon", type="l", lty=1, cex=0.45) # flew in both
# no egg change
...
points(df$mass_diff[egg_nochange_rows], pp[egg_nochange_rows,3], col="maroon", type="l", lty=2) # flew in both
...
points(df$mass_diff[egg_T2rows], pp[egg_T2rows,3], col="maroon", type="l", lty=3) # flew in both

text(0.04,0.6, labels="Flew in T1 only", col="blue")
text(-0.035,0.8, labels="Did Not Fly", col="red")
text(-0.02,0.37, labels="Flew Twice", col="darkred")
legend(0.025, 1.0,
...
gf_point(flight_case ~ mass_diff, data=df, col=~egg_diff) 
```
...
# only laid eggs in T1
plot(df4$mass_diff[egg_T1rows], pp[egg_T1rows,1], ylim=c(0,1), xlim=c(-0.045,0.06), col="red", type="l", lty=1,
     main="Females Only",
     #type="b", cex=0.45, pch=16, 
     ylab="Flight Case Probability", xlab="Change in Mass From T1 to T2 (g)") # flew in neither
points(df4$mass_diff[egg_T1rows], pp[egg_T1rows,2], col="blue", type="l", lty=1, cex=0.45)  # flew in T1 only
points(df4$mass_diff[egg_T1rows], pp[egg_T1rows,3], col="maroon", type="l", lty=1, cex=0.45) # flew in both
# no egg change
points(df4$mass_diff[egg_nochange_rows], pp[egg_nochange_rows,1], col="red", type="l", lty=2) # did not fly in either
points(df4$mass_diff[egg_nochange_rows], pp[egg_nochange_rows,2], col="blue", type="l", lty=2) # flew in T1 only
points(df4$mass_diff[egg_nochange_rows], pp[egg_nochange_rows,3], col="maroon", type="l", lty=2) # flew in both
# only laid eggs in T2
points(df4$mass_diff[egg_T2rows], pp[egg_T2rows,1], col="red", type="l", lty=3) # flew in neither
points(df4$mass_diff[egg_T2rows], pp[egg_T2rows,2], col="blue", type="l", lty=3) # flew in T1 only
points(df4$mass_diff[egg_T2rows], pp[egg_T2rows,3], col="maroon", type="l", lty=3) # flew in both
