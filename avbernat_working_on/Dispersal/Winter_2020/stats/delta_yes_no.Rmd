...
```{r}
df$no_response_b = NA
df$no_response_b[df$num_flew == 0] = 0
df$no_response_b[df$num_flew == 2] = 2
as.factor(df$no_response_b)
```
...
<div align="center">$\log(\hat{\pi_{-1}}/\hat{\pi_0}) = -0.77 + 69.21 x$ 

$\log(\hat{\pi_{1}}/\hat{\pi_0}) = -2.13 + 6.70 x$

$\log(\hat{\pi_2}/\hat{\pi_0}) = 0.42 + 25.32 x$</div><br/>

The estimated log odds that the response is -1 (Flew in T1, not T2) rather than 1 (flew in T2, not T1) equals:

<div align="center">$\log(\hat{\pi_{-1}}/\hat{\pi_1}) = (-0.77 - (-2.13)) + (69.21 - (6.70)) x = 1.36 + 62.51 x$ </div><br/>

The estimated log odds that the response is 2 (Flew in both trials) rather than -1 (flew in T1, not T2) equals:

<div align="center">$\log(\hat{\pi_{-1}}/\hat{\pi_1}) = (0.42 - (-0.77)) + (25.32 - (69.21)) x = 1.19 - 43.89 x$ </div><br/>

The estimated log odds that the response is 2 (Flew in both trials) rather than 1 (flew in T2, not T1) equals:

<div align="center">$\log(\hat{\pi_{-1}}/\hat{\pi_1}) = (0.42 - (-2.13)) + (25.32 - (6.70)) x = 2.55 - 18.62 x$
...
eq2 = "Flew in both rather than T1 only = 1.19 - 43.89 Mass Change"
eq3 = "Flew in both rather than T2 only = 2.55 - 18.62 Mass Change"
...
```{r echo=FALSE}
plot(df$mass_per[Frows], pp[Frows,1], ylim=c(0,1), xlim=c(-40,104), col="red", type="l", ylab="Predicted Probability", xlab="Percent Change in Mass From T1 to T2 (%)", lty=1) # no flight_diff
points(df$mass_per[Mrows], pp[Mrows,1], col="red", type="l", lty=2) # no flight_diff
points(df$mass_per[Frows], pp[Frows,2], col="blue", type="l") # flew in T1 but not T2
points(df$mass_per[Mrows], pp[Mrows,2], col="blue", type="l", lty=2)
points(df$mass_per[Frows], pp[Frows,3], col="darkgreen", type="l") # flew in T2 but not T1
points(df$mass_per[Mrows], pp[Mrows,3], col="darkgreen", type="l", lty=2) 
points(df$mass_per[Frows], pp[Frows,4], col="darkred", type="l") # flew in both
points(df$mass_per[Mrows], pp[Mrows,4], col="darkred", type="l", lty=2) 
