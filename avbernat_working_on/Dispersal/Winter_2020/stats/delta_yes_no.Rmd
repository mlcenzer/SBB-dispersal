...
model <- multinom(flight_case ~ mass_per + sex_c + wing2body_scaled, data = df)
model_table = calculate_P3(model)
```
...
plot5 = function(df, pp, PP, gradient=TRUE, circles=TRUE, stochasticity=TRUE, points=TRUE) {
  ...
  text(13,0.77, labels="Flew Twice", col="darkorange3", cex=c2) # 13,0.77, maroon    
...
color.bar(colorRampPalette(c("black", "grey"))(100), 63)
```
...
ML_eqs = get_significant_modelsf(11) # mass_per 
...
key1 = getting_odds(Odds, effect, pars, ML_eqs[[1]], 2) 

# 2 vs. 0
Odds = c("$\\underline{P(Yi=flew\\,twice)}$", "$P(Yi=did\\, not\\, fly)$", " ")
key2 = getting_odds(Odds, effect, pars, ML_eqs[[2]], 2) 

# -1 vs. 0
Odds = c("$\\underline{P(Yi=flew\\, in\\, T1\\, only)}$", "$P(Yi=did\\, not\\, fly)$", " ")
key3 = getting_odds(Odds, effect, pars, ML_eqs[[2]], 1) 
