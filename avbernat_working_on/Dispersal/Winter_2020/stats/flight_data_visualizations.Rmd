...
data_all <- read_flight_data("data/all_flight_data-Winter2020.csv")
```
...
source("src/categorize_binomial_data.R") # categorize_binomial_data2.R works too
```
...
p1 <- as.grob(expression(plot(as.matrix(cat_data1[1]), 
     ...
     ylab="Sample Proportion of Yes Flew", xlab="Minutes From Incubtation Start (min) ", main="Observed proportions of yes flew by minutes from incubation start")
))
...
p2 <- as.grob(expression(
  plot(as.matrix(cat_data2[1]), 
     ...
     ylab="Sample Proportion of Yes Flew", xlab="Minutes From Trial Start (min)", main="Observed proportions of yes flew by minutes from trial start") %>%
...
fit <- lm(`Sample Proportion`~ min_from_IncStart, data=cat_data1)
...
plot(cat_data1$min_from_IncStart,cat_data1$`Sample Proportion`, xlab= "Starting Time", ylab="Proportion of 24-H Day Spent Flying")
...
m <- quap(
  alist(
    `Sample Proportion` ~ dnorm(mu, sigma),
...
# Plot Log
plot( cat_data2$log_min_duration , cat_data2$`Sample Proportion` , col=col.alpha(rangi2,0.7),
...
# Plot Non-log
plot( cat_data2$minute_duration , cat_data2$`Sample Proportion` , col=col.alpha(rangi2,0.7), 
      ylab="Sample Proportion of Yes Flew", 
...
all_fit <-lm(`Sample Proportion`~ mass, data=cat_data3) 
