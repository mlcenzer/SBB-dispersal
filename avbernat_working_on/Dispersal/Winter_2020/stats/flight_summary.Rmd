...
data_all = data[[1]]
data_tested = data[[2]]
d = create_delta_data(data_tested, remove_bugs_tested_once = FALSE) # keep bugs only tested once
```
...
d$mass_block=round(d$average_mass/0.005)*0.005 # 0.005 g blocks 
...
lines(x=xy.coords(x=c(1,1), y=c(data_temp$CI$lower[1], data_temp$CI$upper[1])), lwd=3, col=sex_colors[1])
lines(x=xy.coords(x=c(2,2), y=c(data_temp$CI$lower[2], data_temp$CI$upper[2])), lwd=3, col=sex_colors[2])
...
dd=aggregate(flew_prob~sex*wing2body_block, data=d, FUN=mean)
dd$n=aggregate(flew_prob~sex*wing2body_block, data=d, FUN=length)$flew_prob
...
plot(dd$flew_prob~dd$wing2body_block, 
     ylab="Flight probability", xlab="wing-to-body ratio", 
     pch=19, col=c("red","blue")[as.factor(dd$sex)], 
     cex=c1, ylim=c(0,1))
...
dd=aggregate(flew_prob~sex*mass_block, data=d, FUN=mean)
dd$n=aggregate(flew_prob~sex*mass_block, data=d, FUN=length)$flew_prob
...
plot(dd$flew_prob~dd$mass_block, 
     ylab="Flight probability", 
     xlab="Mass (g)", 
     pch=19, 
     col=sex_colors[as.factor(dd$sex)], 
     cex=c1,
...
```{r warning=FALSE}
head(pp <- fitted(M1),3) # compute fitted values of the best fit model without wing-to-body ratio
```
...
```{r warning=FALSE}
head(pp <- fitted(M2), 3) # compute fitted values of the best fit model with wing-to-body ratio
```
...
```{r results="hold"}
M3 <- multinom(flight_case ~ mass_per + egg_case, data = df) # same top model 
...
counts <- table(data_fem$eggs_b, data_fem$trial_type) 
counts <- counts[1:2,2:3]
