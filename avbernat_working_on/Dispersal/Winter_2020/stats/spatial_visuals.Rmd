...
df_final_pop <- st_make_valid(df_transformed) %>%
...
tm_shape(combined_filtered, bbox=keys_bb) + # only place where it would matter is in the keys.
    tm_graticules(n.x = 3, n.y = 7, labels.format = list(digits=1), col= "azure2") +
    tm_polygons("counties", labels = NULL, legend.show = FALSE) +
...
tm_shape(site_buffer_data) + # NKL_buffer
  tm_fill("distance", alpha=0.02, legend.show = FALSE) + # can change the color, col = "blue"
tm_shape(dispersal_filtered) +
  tm_dots(alpha = 0.9, title = 'Collection Sites', labels = "geometry") +
  tm_text("zmap_site", size = 0.4, xmod = -1.7, ymod = 0.15)
