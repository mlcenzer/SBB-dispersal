...
```{r}
anova(m5, m231, test="Chisq") # Adding Y does not improve fit
```

```{r}
femm_eggmorph <- lmer(mass_c ~ eggs_b + thorax_c + (1|ID), data=data_fem)
tidy_regression(femm_eggmorph, is_color=output_col)
```

* if female laid eggs that day, then mass is larger
* the larger the thorax, the heavier the female
...
```{r echo=FALSE}
###### No effect of chamber
tidy_regression(lmer(mass_c~chamber + (1|days_from_start_c), data=data_T1), is_color=output_col) # (1|population) variance near zero so can remove

####### No effect of test date
tidy_regression(lmer(mass_c~days_from_start_c + (1|chamber), data=data_T1), is_color=output_col)
...
```{r, echo=FALSE}
####### Effect of number of eggs laid
tidy_regression(lmer(mass_c~total_eggs + (1|chamber), data=data_T1), is_color=output_col) 
...
```{r, echo=FALSE}
####### Effect of beak length
tidy_regression(lmer(mass_c~beak_c + (1|chamber), data=data_T1), is_color=output_col) 

####### Effect of thorax length
tidy_regression(lmer(mass_c~thorax_c + (1|chamber), data=data_T1), is_color=output_col)

####### Effect of body length
tidy_regression(lmer(mass_c~body_c + (1|chamber), data=data_T1), is_color=output_col)

####### Effect of wing length
tidy_regression(lmer(mass_c~wing_c + (1|chamber), data=data_T1), is_color=output_col)
```
...
anova(m12, m13, test="Chisq") # Replacing B with A does not improve fit
```
