...
tidy_regression_bw(lmer(mass_c~chamber + (1|ID) + (1|trial_type), data=data_tested)) # shouldn't matter because this measurement was taken seperate from the chamber.

####### No effect of test date
tidy_regression_bw(lmer(mass_c~days_from_start_c + (1|ID) + (1|trial_type), data=data_tested)) # t val = 1.975

####### No effect of test time
tidy_regression_bw(lmer(mass_c~min_from_IncStart_c + (1|ID) + (1|trial_type), data=data_tested)) 
```
...
tidy_regression_bw(lmer(mass_c~total_eggs + (1|ID), data=data_tested)) # model didn't converge with (1|trial_type)

####### Effect of whether eggs were laid or not
tidy_regression_bw(lmer(mass_c~eggs_b + (1|ID) + (1|trial_type), data=data_tested))
```
...
tidy_regression_bw(lmer(mass_c~beak_c + (1|ID), data=data_tested)) # model didn't converge with (1|trial_type)

####### Effect of thorax length
tidy_regression_bw(lmer(mass_c~thorax_c + (1|ID) + (1|trial_type), data=data_tested))

####### Effect of body length
tidy_regression_bw(lmer(mass_c~body_c + (1|ID) + (1|trial_type), data=data_tested))

####### Effect of wing length
tidy_regression_bw(lmer(mass_c~wing_c + (1|ID) + (1|trial_type), data=data_tested))
...
tidy_regression_bw(model_alldata)
```
...
source("AICprobabilities.R")
source("generic models-gaussian glmer 2-RF + 4-FF.R") # multiple models failed to converge
AICs <- sort(summary$AIC)
models_init <- sort(P, decreasing=TRUE, index.return=TRUE)
top <- length(models_init$x[which(models_init$x>0.05)])
...
```{r}
#wing2body
data_tested$wing2body <- data_tested$wing / data_tested$body
data_tested$wing2body_c <- data_tested$wing2body - mean(data_tested$wing2body, na.rm=TRUE)
...
source("AICprobabilities.R")
source("generic models-gaussian glmer 2-RF + 3-FF.R") # 4 models failed to converge
AICs <- sort(summary$AIC)
models_init <- sort(P, decreasing=TRUE, index.return=TRUE)
top <- length(models_init$x[which(models_init$x>0.05)])
...
lm_plot(data_tested, "sex_c","mass_c", "Sex", "Mass (g)", "Mass by Sex") 
axis(1, at = seq(-1,1,.5),labels=c("M", " ", " ", " ", "F"))
#dev.off()

#pdf("lm-massbyhost-model.pdf", width=6, height=6)
lm_plot(data_tested, "host_c","mass_c", "Host Plant", "Mass (g)", "Mass by Host Plant") 
...
g2 <- grid.arrange(p4,p5,p6,p7, ncol=2, top="All Data")
...
```{r echo=FALSE}
###### No effect of chamber
tidy_regression_bw(lmer(mass_c~chamber + (1|ID) + (1|trial_type), data=data_fem)) # shouldn't matter because this measurement was taken seperate from the chamber.

####### Effect of test date
tidy_regression_bw(lmer(mass_c~days_from_start_c + (1|ID), data=data_fem)) # t val = 2.38 - makes sense because female mass would fluctuate the most

####### No effect of test time
tidy_regression_bw(lmer(mass_c~min_from_IncStart_c + (1|ID) + (1|trial_type), data=data_fem)) 
```
...
```{r, echo=FALSE}
####### Effect of number of eggs laid
tidy_regression_bw(lmer(mass_c~total_eggs + (1|ID) + (1|trial_type), data=data_fem)) 

####### Effect of whether eggs were laid or not
tidy_regression_bw(lmer(mass_c~eggs_b + (1|ID), data=data_fem))
```
...
```{r, echo=FALSE}
####### Effect of beak length
tidy_regression_bw(lmer(mass_c~beak_c + (1|ID) + (1|trial_type), data=data_fem)) # model didn't converge with (1|trial_type)

####### Effect of thorax length
tidy_regression_bw(lmer(mass_c~thorax_c + (1|ID) + (1|trial_type), data=data_fem))

####### Effect of body length
tidy_regression_bw(lmer(mass_c~body_c + (1|ID) + (1|trial_type), data=data_fem))

####### Effect of wing length
tidy_regression_bw(lmer(mass_c~wing_c + (1|ID) + (1|trial_type), data=data_fem))
...
```{r}
anova(m3, m10, test="Chisq") # Adding D marginally improves the fit
```
...
data <- data %>%
  filter(!is.na(A))
source("AICprobabilities.R")
source("generic models-gaussian glmer 2-RF + 4-FF.R") # 8 models failed
AICs <- sort(summary$AIC)
models_init <- sort(P, decreasing=TRUE, index.return=TRUE)
top <- length(models_init$x[which(models_init$x>0.05)])
...
probs <- models_init$x[1:top]
rbind(AICs, models, probs) 
# holds a combined 94% probability when remove (1|trial_type)
```
...
tidy_regression_bw(femm_morph_model)
```
...
source("plotting-lm2.R") 
lm_plot(data_fem, "eggs_b", "mass_c", "Eggs Laid", "Mass (g)", "Mass by Egg Laid on Test Day")
axis(1, at = seq(-1,1,.5),labels=c(" ", " ", "M ", " ", "F"))
```
...
```{r echo=FALSE}
###### No effect of chamber
tidy_regression_bw(lmer(mass_c~chamber + (1|ID), data=data_male)) # shouldn't matter because this measurement was taken seperate from the chamber.

####### No effect of test date
tidy_regression_bw(lmer(mass_c~days_from_start_c + (1|ID) + (1|trial_type), data=data_male))

####### No effect of test time
tidy_regression_bw(lmer(mass_c~min_from_IncStart_c + (1|ID) + (1|trial_type), data=data_male)) 
```
...
```{r, echo=FALSE}
####### Effect of beak length
tidy_regression_bw(lmer(mass_c~beak_c + (1|ID) + (1|trial_type), data=data_male)) 

####### Effect of thorax length
tidy_regression_bw(lmer(mass_c~thorax_c + (1|ID) + (1|trial_type), data=data_male))

####### Effect of body length
tidy_regression_bw(lmer(mass_c~body_c + (1|ID) + (1|trial_type), data=data_male))

####### Effect of wing length
tidy_regression_bw(lmer(mass_c~wing_c + (1|ID) + (1|trial_type), data=data_male))
...
models <- lapply(models_init$ix[1:top],1,FUN=as.integer)
probs <- models_init$x[1:top]
rbind(AICs, models, probs) # m5 holds 89% of probability
```
...
```{r}
anova(m5, m7, test="Chisq") # Replacing B with D does not improve fit
```

```{r}
male_morph_model <- lmer(mass_c ~ beak_c + thorax_c + (1|ID), data=data_male)
tidy_regression_bw(male_morph_model)
```
...
```{r echo=FALSE}
###### No effect of chamber
tidy_regression_bw(lmer(mass_c~chamber + (1|ID), data=data_T1)) # (1|population) variance near zero so can remove

####### No effect of test date
tidy_regression_bw(lmer(mass_c~days_from_start_c + (1|ID), data=data_T1))

####### No effect of test time
tidy_regression_bw(lmer(mass_c~min_from_IncStart_c + (1|chamber), data=data_T1)) 
```
...
```{r, echo=FALSE}
####### Effect of number of eggs laid
tidy_regression_bw(lmer(mass_c~total_eggs + (1|ID), data=data_T1)) 

####### Effect of whether eggs were laid or not
tidy_regression_bw(lmer(mass_c~eggs_b + (1|chamber), data=data_T1))
```
...
```{r, echo=FALSE}
####### Effect of beak length
tidy_regression_bw(lmer(mass_c~beak_c + (1|ID), data=data_T1)) 

####### Effect of thorax length
tidy_regression_bw(lmer(mass_c~thorax_c + (1|ID), data=data_T1))

####### Effect of body length
tidy_regression_bw(lmer(mass_c~body_c + (1|ID), data=data_T1))

####### Effect of wing length
tidy_regression_bw(lmer(mass_c~wing_c + (1|ID), data=data_T1))
...
- m74 <- R ~ A * B + B * C + B * D + (1 | X)

```{r}
T1_morph_model <- lmer(mass_c ~ beak_c * wing_c + thorax_c * body_c + (1 | days_from_start_c), data=data_T1)
tidy_regression_bw(T1_morph_model)
```

```{r}
#wing2body
data_T1$wing2body <- data_T1$wing / data_T1$body
data_T1$wing2body_c <- data_T1$wing2body - mean(data_T1$wing2body, na.rm=TRUE)
...
```{r echo=FALSE}
###### No effect of chamber
tidy_regression_bw(lmer(mass_c~chamber + (1|min_from_IncStart_c), data=data_T2))

####### No effect of test date
tidy_regression_bw(lmer(mass_c~days_from_start_c + (1|min_from_IncStart_c), data=data_T2))

####### No effect of test time
tidy_regression_bw(lmer(mass_c~min_from_IncStart_c + (1|chamber), data=data_T2)) 
```
...
```{r, echo=FALSE}
####### Effect of number of eggs laid
tidy_regression_bw(lmer(mass_c~total_eggs + (1|days_from_start_c), data=data_T2)) 

####### Effect of whether eggs were laid or not
tidy_regression_bw(lmer(mass_c~eggs_b + (1|days_from_start_c), data=data_T2))
```
...
```{r, echo=FALSE}
####### Effect of beak length
tidy_regression_bw(lmer(mass_c~beak_c + (1|days_from_start_c), data=data_T2)) 

####### Effect of thorax length
tidy_regression_bw(lmer(mass_c~thorax_c + (1|days_from_start_c), data=data_T2))

####### Effect of body length
tidy_regression_bw(lmer(mass_c~body_c + (1|days_from_start_c), data=data_T2))

####### Effect of wing length
tidy_regression_bw(lmer(mass_c~wing_c + (1|days_from_start_c), data=data_T2))
...
```{r}
T2_model <- lmer(mass_c ~ sex_c + (1|days_from_start_c), data=data_T2)
tidy_regression_bw(T2_model)
```
...
data <- data %>%
  filter(!is.na(A))
source("AICprobabilities.R")
source("generic models-gaussian glmer 1-RF + 4-FF.R") # 1 model failed to converge
AICs <- sort(summary$AIC)
models_init <- sort(P, decreasing=TRUE, index.return=TRUE)
top <- length(models_init$x[which(models_init$x>0.05)])
...
tidy_regression_bw(T2_morph_model)
```

```{r}
#wing2body
data_T2$wing2body <- data_T2$wing / data_T2$body
data_T2$wing2body_c <- data_T2$wing2body - mean(data_T2$wing2body, na.rm=TRUE)
