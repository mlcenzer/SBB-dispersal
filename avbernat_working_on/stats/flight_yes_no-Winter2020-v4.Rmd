...
temp_filepath = "~/Desktop/Rstats-winter2020/complete_flight_data-Winter2020.csv"
file_name = "clean_flight_data.R"
...
```{r}
model_T1<-glm(flew_b~sex_c, family=binomial, data=data_T1)
summary(model_T1)
```
* strong negative effect of sex, where if you are female you are less likely to fly.
* no longer an effect of host or distance from sympatric zone.

```{r}
model_T1_final <-glmer(flew_b~sex_c + (1|population), family=binomial, data=data_T1) #  boundary (singular) fit: see ?isSingular error
