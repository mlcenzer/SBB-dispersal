...
source("src/generic models-Gamma glmer 3-FF log link.R") # 9 models failed to converge
...
rbind(AICs, models, probs) # No model with chamber was in the top 5.
```

- m2 <- glm(formula = R ~ B, family = Gamma(link = "log"), data = data)
- m6 <- glm(formula = R ~ B + C, family = Gamma(link = "log"), data = data)
- m4 <- glm(formula = R ~ A + B, family = Gamma(link = "log"), data = data)
- m1 <- glm(formula = R ~ A, family = Gamma(link = "log"), data = data)
- m3 <- glm(formula = R ~ C, family = Gamma(link = "log"), data = data)
...
source("src/AICprobabilities.R")
source("src/generic models-Gamma glmer 4-FF log link.R") # most models failed to converge
AICs <- sort(summary$AIC)
models_init <- sort(P, decreasing=TRUE, index.return=TRUE)
top <- length(models_init$x[which(models_init$x>0.05)])
...
source("src/AICprobabilities.R")
source("src/generic models-Gamma glmer 3-FF log link.R") # about 3 models failed to converge
AICs <- sort(summary$AIC)
models_init <- sort(P, decreasing=TRUE, index.return=TRUE)
top <- length(models_init$x[which(models_init$x>0.05)])
...
- m4 <- glm(formula = R ~ A + B, family = Gamma(link = "log"), data = data)
- m6 <- glm(formula = R ~ B + C, family = Gamma(link = "log"), data = data)
- m5 <- glm(formula = R ~ A + C, family = Gamma(link = "log"), data = data)
- m9 <- glm(formula = R ~ A * C, family = Gamma(link = "log"), data = data)
...
source("src/AICprobabilities.R")
source("src/generic models-Gamma glmer 3-FF log link.R")
AICs <- sort(summary$AIC)
models_init <- sort(P, decreasing=TRUE, index.return=TRUE) # singular fit warning b/c chamber var close to 0
top <- length(models_init$x[which(models_init$x>0.05)])
...
source("src/AICprobabilities.R")
source("src/generic models-Gamma glmer 4-FF log link.R")
AICs <- sort(summary$AIC)
models_init <- sort(P, decreasing=TRUE, index.return=TRUE) # singular fit warning b/c chamber var close to 0
top <- length(models_init$x[which(models_init$x>0.05)])
...
null model is the best model again
...
source("src/AICprobabilities.R")
#source("generic models-Gamma glmer 3-FF log link.R") # doesn't work for T2 but does for T1
```
