...
```{r message=FALSE}
data<-data.frame(R=d_fem$distance, A=d_fem$min_from_IncStart_c, B=d_fem$sym_dist, C=d_fem$mass_c, X=d_fem$chamber)

source("src/compare_models.R")
errors <- withWarnings(model_comparisonsAIC("src/generic models-Gamma glmer 3-FF log link.R"))
cat("Number of models that failed to converge:", length(errors$warnings)) # 11 models failed to converge
```

```{r}
anova(m3, m5, test="Chisq") # Adding C does not improve fit
anova(m3, m6, test="Chisq") # Adding B does not improve fit
```
...
```{r message=FALSE}
data<-data.frame(R=d_male$distance, A=d_male$min_from_IncStart_c, B=d_male$sym_dist, C=d_male$mass_c, X=d_male$chamber)

source("src/compare_models.R")
errors <- withWarnings(model_comparisonsAIC("src/generic models-Gamma glmer 3-FF log link.R"))
cat("Number of models that failed to converge:", length(errors$warnings)) # 25 models failed to converge
```
...
```{r}
best.fit <- glmer(distance ~ mass_c + (1|chamber), family = Gamma(link = "log"), data = d_fem) 
tidy_regression(best.fit, is_color=output_col)
```
