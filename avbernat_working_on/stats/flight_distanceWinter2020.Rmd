...
cat("Number of models that failed to converge:", length(errors$warnings)) # a lot of models failed to converge
```
...
```{r, message=FALSE}
data<-data.frame(R=d_fem$distance, A=d_fem$host_c, B=d_fem$sex_c, C=d_fem$sym_dist, X=d_fem$chamber)
...
```{r, message=FALSE}
data<-data.frame(R=d_fem$distance, A=d_fem$host_c, B=d_fem$sex_c, C=d_fem$sym_dist, D=d_fem$mass_c, X=d_fem$chamber)
...
```{r message=FALSE}
data<-data.frame(R=d_fem$distance, A=d_fem$min_from_IncStart_c, B=d_fem$sex_c, C=d_fem$sym_dist, D=d_fem$mass_c, X=d_fem$chamber)
...
```{r, message=FALSE}
data<-data.frame(R=d_male$distance, A=d_male$host_c, B=d_male$sex_c, C=d_male$sym_dist, X=d_male$chamber)
...
```{r, message=FALSE}
data<-data.frame(R=d_male$distance, A=d_male$host_c, B=d_male$sex_c, C=d_male$sym_dist, D=d_male$mass_c, X=d_male$chamber)
...
```{r message=FALSE}
data<-data.frame(R=d_male$distance, A=d_male$min_from_IncStart_c, B=d_male$sex_c, C=d_male$sym_dist, D=d_male$mass_c, X=d_male$chamber)
...
```{r}
best.fit <- glmer(distance ~ min_from_IncStart_c * mass_c + (1|chamber), family = Gamma(link = "log"), data = d_fem) 
tidy_regression(best.fit, is_color=output_col)
```

* no significant effects - also the model doesn't make too much sense 
...
data<-data.frame(R=d_male$distance, A=d_male$wing2body, B=d_male$thorax_c, C=d_male$mass_c, X=d_male$chamber)
...
```{r}
best.fit <- glmer(distance ~ mass_c * thorax_c + (1|chamber), family = Gamma(link = "log"), data = d_male)  # model failed to converge
tidy_regression(best.fit, is_color=output_col)
```
