...
num_tested_fem <- nrow(data_tested[data_tested$sex == "F",])
num_tested_male <- nrow(data_tested[data_tested$sex == "M",])

table0 <- data.frame("All" = c(num_fem, num_male), "Tested" = c(num_tested_fem, num_tested_male))
...
table1 <- data.frame(pops, female_sizes, male_sizes)
colnames(table1) <- c("Population", "Females", "Males")

kable(table1) %>%
  kable_styling(bootstrap_options = "striped", "hover", full_width = F)  %>%
  add_header_above(c(" ", "Sex Sample Sizes" = length(table1) - 1)) %>% 
  add_header_above(c("Tested Bugs By Pop and Sex" = 3 ))
```
...
mF <-  round(mean(data_all[data_all$sex == "F",]$mass, na.rm=TRUE), 3)
sdF <- round(sd(data_all[data_all$sex == "F",]$mass, na.rm=TRUE), 3)
mM <- round(mean(data_all[data_all$sex == "M",]$mass, na.rm=TRUE), 3)
sdM <- round(sd(data_all[data_all$sex == "M",]$mass, na.rm=TRUE), 3)

table3 <- data.frame( "mean" = c(mF, mM), 
            "std" = c(sdF, sdM), 
            "sample size" = c(countF, countM), 
            row.names = c("F", "M"))
