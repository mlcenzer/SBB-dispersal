...
lm_plot <- function(d, explanatory_var, response_var, xl, yl, title) {
  ...
  text(mean(x) + 0.3*sd(x), mean(y) + 3*sd(y), p, col= color, cex=0.9)
  text(mean(x) + 0.3*sd(x), mean(y) + 2.6*sd(y), corr, col= color, cex=0.9)
  text(mean(x) + 0.3*sd(x), mean(y) + 3.4*sd(y), eq, col= color, cex=0.9)
