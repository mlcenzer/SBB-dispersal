...
source("src/AICprobabilities.R")
source("src/generic models-gaussian glmer 2-RF + 4-FF.R") 
AICs <- sort(summary$AIC)
models_init <- sort(P, decreasing=TRUE, index.return=TRUE)
top <- length(models_init$x[which(models_init$x>0.05)])
...
- m72 <- R ~ A * B + A * D + B * D + (1 | X)
- m185 <- R ~ A * B + A * D + B * D + (1 | Y)
- m154 <- R ~ A * B + A * D + (1 | Y)
- m41 <-  R ~ A * B + A * D + (1 | X)
- m298 <- R ~ A * B + A * D + B * D + (1 | X) + (1 | Y)
- m267 <- R ~ A * B + A * D + (1 | X) + (1 | Y)
...
- m193 <- R ~ A * C + B * C + B * D + (1 | Y)
- m306 <- R ~ A * C + B * C + B * D + (1 | X) + (1 | Y)
