...
* marginal negative effect of body where the larger the body the less likely to fly
...
* no effect of body*wing. We know body and wing are closely correlated to one another, so the could cancel each other out. Meaning, that if you have a longer body then you have more mass and even if you have the wings to fly you'll be less likely to fly.
...
```{r}
morph_model_T2 <- glm(flew_b ~ beak_c * wing_c + body_c * wing_c, family = binomial, data = data_T2)
...
* positive effect of beak:wing where the longer the beak and wing the more likely the bug is to fly
* negative effect of wing and body where the longer the wing and body the less likely the bug is to fly

```{r}
morph_model_T2glmer <- glmer(flew_b ~ beak_c * wing_c + body_c * wing_c + (1|chamber), family = binomial, data = data_T2) # (1|days_from_start_c) and (1|population)variance = 0, chamber did not have variance at 0
...
R = data_fem$flew_b
A = data_fem$host_c
B = data_fem$w_morph_c # before had sym_dist but now leave out because did not have significant effects or interactions within the total model dataset.
C = data_fem$mass_c
...
anova(m1, m3, test="Chisq") # Adding B does marginally improve improve fit
anova(m2, m3, test="Chisq") # Adding A does not improve fit
```

```{r echo=FALSE}
model_fem <-glm(flew_b~w_morph_c, family=binomial, data=data_fem)
tidy_regression(model_fem, is_color=output_col) 
```

* marginal positive effect of wing morph where if L wing (rather than LS = 0 or S = -1) then female more likely to fly.
...
R = data_fem$flew_b
A = data_fem$host_c
B = data_fem$w_morph_c # used to be sym_dis
C = data_fem$mass_c
...
```{r echo=FALSE}
model_fem_mass <-glm(flew_b~w_morph_c + mass_c, family=binomial, data=data_fem)
...
* marginal positve effect of wing morph where if L (rather than LS or S) then more likely to disperse.
...
A = data_fem$host_c
B = data_fem$mass_c
C = data_fem$w_morph_c # replaced sym_dist
X = data_fem$trial_type  # variance is zero
...
anova(m6, m10, test="Chisq") # Adding B*C does not improve fit
anova(m6, m7, test="Chisq") # Adding A does not improve fit
anova(m2, m6, test="Chisq") # Adding B does improve fit
```
...
* [glmer() A=beak, B=thorax, C=body, D=wing, X=days_from_start, Y=trial_type](#morph_glmer)

* [glmer() A=wing2body, B=days_from_start, X=trial_type](#morph_glmer3)
...
## Changed the effects slightly and in turn made host marginally significant significant but not a better fitting model.
```
...
```{r}
# Remnant Code: best model when you treat days_from_start as a random effect
...
#### glmer() A=beak, B=thorax, C=body, D=wing, X=days_from_start, Y=trial_type
...
source("src/compare_models.R")
model_comparisonsAIC("src/generic models-binomial glmer 1-RF + 4-FF-noD-interactions.R")
```

```{r}
male_morph.glmer <- glmer(flew_b ~ beak_c * body_c + beak_c * wing_c + body_c * wing_c + days_from_start_c + (1|trial_type), family=binomial, data=data_male) # model failed to converge if use (1|ID) but not if use (1|trial_type)
...
#### glmer() A=wing2body, B=days_from_start, X=trial_type
...
```{r}
male_morph.glmer <- glmer(flew_b ~ wing2body_c + days_from_start_c + (1|trial_type), family=binomial, data=data_male) 
tidy_regression(male_morph.glmer, is_color=output_col) # best fit model from glmer()
```
