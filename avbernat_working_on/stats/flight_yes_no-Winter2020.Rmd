...
tidy_regression(glm(cbind(num_flew, num_notflew)~average_mass, data=d, family=binomial), is_color=output_col) ####MLC: updated to binom

####### Effect of number of eggs laid
tidy_regression(glm(cbind(num_flew, num_notflew)~total_eggs, data=d, family=binomial), is_color=output_col) ####MLC: updated to binom

####### Can't test eggs_b b/c it's date specific

####### Effect of beak length
tidy_regression(glm(cbind(num_flew, num_notflew)~beak_c, data=d, family=binomial), is_color=output_col) ####MLC: updated to binom

####### Effect of thorax length
tidy_regression(glm(cbind(num_flew, num_notflew)~thorax_c, data=d, family=binomial), is_color=output_col) ####MLC: updated to binom

####### Effect of body length
tidy_regression(glm(cbind(num_flew, num_notflew)~body_c, data=d, family=binomial), is_color=output_col) ####MLC: updated to binom

####### No effect of wing length
tidy_regression(glm(cbind(num_flew, num_notflew)~wing_c, data=d, family=binomial), is_color=output_col) ####MLC: updated to binom
...
```{r}
nomass_model <- glmer(cbind(num_flew, num_notflew) ~ sex_c * host_c + (1|population), data=d, family=binomial)
...
* no effect of host
* marginal positive effect of sex * host where if from GRT and F then more likely to fly
...
```{r}
mass_model <- glmer(cbind(num_flew, num_notflew) ~ host_c * average_mass + sym_dist * average_mass +  sex_c + (1|population), data=d, family=binomial)
tidy_regression(mass_model, is_color=output_col)
```
* no effect of average mass
* negative effect of host where if from GRT then less times will fly. 
* positive effect of sym_dist whre if farther from Homestead then more times will fly

* strong positive host*aveage_mass interaction where if from GRT and heavy then fly more times
* strong negative sym_dist*aveage_mass interaction where if farther from Homestead and heavy then fly less.
...
```{r}
morph_model <- glmer(cbind(num_flew, num_notflew) ~ thorax_c * body_c + body_c * wing_c + (1|population), data=d, family=binomial)
...
```{r}
morph_model <- glmer(cbind(num_flew, num_notflew) ~ wing2body_c * thorax_c + (1|population), data=d, family=binomial)
tidy_regression(morph_model, is_color=output_col)
```
...
source("src/compare_models.R")
errors <- withWarnings(model_comparisonsAIC("src/generic models-binomial glmer 2R ~ 1 RF + 3-FF.R"))
length(errors$warnings) # 7 models failed to converge
```
...
```{r}
multi_model <- glmer(cbind(num_flew, num_notflew) ~ thorax_c * wing2body_c + wing2body_c * average_mass + (1|population), data=d, family=binomial) # error | did not converge 
...
```{r}
multi_model <- glmer(cbind(num_flew, num_notflew) ~ thorax_c * wing2body_c + sex_c + (1|population), data=d, family=binomial) 
tidy_regression(multi_model, is_color=output_col)
```
...
source("src/compare_models.R")
errors <- withWarnings(model_comparisonsAIC("src/generic models-binomial glmer 2R ~ 1-RF + 4-FF.R"))
length(errors$warnings) # 62 models failed
```
...
```{r}
multi_model <- glmer(cbind(num_flew, num_notflew) ~ thorax_c * wing2body_c + wing2body_c * average_mass + sex_c + (1|population), data=d, family=binomial) # model failed to converge 
#multi_model <- glmer(cbind(num_flew, num_notflew) ~ thorax_c * wing2body_c + wing2body_c * average_mass + sex_c * wing2body_c+ (1|population), data=d, family=binomial) # model failed to converge
tidy_regression(multi_model, is_color=output_col)
```
