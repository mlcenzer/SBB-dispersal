...
####### Effect of body length
tidy_regression(lmer(mass_c~body_c + (1|ID) + (1|trial_type), data=data_fem, REML=FALSE), is_color=output_col)

####### Effect of wing length
tidy_regression(lmer(mass_c~wing_c + (1|ID) + (1| trial_type), data=data_fem, REML=FALSE), is_color=output_col) 
```
...
```{r echo=FALSE}
###### No effect of chamber
tidy_regression(lmer(mass_c~chamber + (1|ID) + (1|trial_type), data=data_male, REML=FALSE), is_color=output_col) 
...
```{r}
anova(m3, m4, test="Chisq") # Adding A*B marginally improves fit
```
...
```{r}
anova(m74, m94, test="Chisq") # Adding A*C marginally improves fit
```

```{r}
male_morph_model <- lmer(mass_c ~ beak_c * thorax_c + beak_c * wing_c + thorax_c * wing_c + thorax_c * body_c + (1|ID), data=data_male, REML=FALSE)
tidy_regression(male_morph_model, is_color=output_col)

male_morph_model <- lmer(mass_c ~ beak_c * thorax_c + beak_c * wing_c + thorax_c * wing_c + thorax_c * body_c + (1|ID) + (1|trial_type), data=data_male, REML=FALSE)
tidy_regression(male_morph_model, is_color=output_col)
```
...
* positive effect of body, where the longer the body the heavier the bug

* negative effect of beak*thorax
* no effect of beak*wing
* negative effect of thorax*wing
