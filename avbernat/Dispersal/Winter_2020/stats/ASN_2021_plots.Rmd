...
data_all <- data[[1]]
data_tested <- data[[2]]
d_all <- create_delta_data(data_tested, remove_bugs_tested_once=FALSE)
```


```{r}
##MC: use average days as covariate
d_all$avg_days_c<-d_all$avg_days-mean(d_all$avg_days, na.rm=TRUE)

##this transformation gets avg_mass to act normal and not give haywire effect estimates
d_all$avg_mass_trans<-log(sqrt(d_all$avg_mass))-mean(log(sqrt(d_all$avg_mass)), na.rm=TRUE)
...
d$mass_block<-round(d$avg_mass/0.005)*0.005
