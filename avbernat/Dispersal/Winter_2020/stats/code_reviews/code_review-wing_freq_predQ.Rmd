...
#manually calculating predicted probabilities
intercept = model$coefficients[1]
coeff = model$coefficients[2]
month = seq(min(raw_data$month_of_year), max(raw_data$month_of_year),len=500)
p = exp(intercept + coeff*month)/ (1 + exp(intercept + coeff*month))

c(min(p), max(p)) # check, all good!
