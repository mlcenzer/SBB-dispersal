...
```{r}
# filtered_data <- data_all[,c("ID", "filename", "chamber", "set_number", "average_speed",
...
get_distance_buffer <- function(data, site_name) { 
  ...
  d_buff = d_buff[,c("distance", "total_flight_time", "population", "site", "geometry")]
}
...
```{r}
ggplot(site_buffer_data, aes(x=distance, y=total_flight_time)) + 
  geom_point(aes(size=distance), label = "max distance")

boxplot(site_buffer_data[,c("site", "distance")])

site_buffer_data %>% group_by(site, population) 
maxdistances = site_buffer_data %>% group_by(site, population)   %>%
  summarise(max = max(distance, na.rm=TRUE))

maxdistances[order(maxdistances$max),]

boxplot(site_buffer_data$distance ~ site_buffer_data$site)
```
...
tm_shape(Florida_ShapeFile.UTM) +
  #tm_graticules(n.x = 3, n.y = 7, labels.format = list(digits=1), col= "grey79") +
  tm_layout (frame = FALSE, bg.color = "transparent") +
  tm_polygons(col="white")  +
tm_shape(site_buffer_data) + 
  tm_fill("distance", alpha=0.05, legend.show = FALSE, col="#699e37") 
```
...
tm_shape(Keys.UTM) + 
  ...
  tm_polygons(col="white")  +
tm_shape(site_buffer_data) + 
  tm_fill("distance", alpha=0.015, legend.show = FALSE, col="#7ecbde") 
```
...
```{r}
zmap.filename = "keys_dist_map2.png"
...
map.filename = "propflew_map_full.png"
