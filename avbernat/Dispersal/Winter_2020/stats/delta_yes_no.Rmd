...
```{r}
d[d$sex =="F" & d$flight_case == 1,] # no females where only flew in T2

# d[d$sex =="F" & d$flight_case == 0,]$flight_case[1:15] = 1
# 
# d[d$sex =="F" & d$flight_case == 1,]
```
...
odds_table2b = rbind(odds_table2[1,], fems, males, wings)
...
exp(-0.57)
t1f = exp(23.74/100 + -0.57) # T1 F
exp(0.57)
t1m = exp(23.74/100 + 0.57) # T1 M

exp(-0.76)
t2f = exp(28.09/100 + -0.76) # T2 F
exp(0.76)
t2m = exp(28.09/100 + 0.76) # T2 M

(0.4676664 / 0.6193405 - 1) * 100
(2.138276/ 2.831764 - 1) *100
...
(t2f/t1f - 1) *100 #14% more likely to fly in T1 than twice
(t1m/t2m - 1) *100 # 21% more likely to fly twice than fly in T1

(t1f/t1m - 1) * 100
(t2f/t2m - 1) * 100

exp(23.74*0.025 + -0.57)
exp(23.74/100)
```
...
odds1 = get_95_CI(odds_table2b[,3], odds_table2b[,4], odds_table2b$Transformation, odds_table2b$OddsTransformation)
odds2 = get_95_CI(odds_table2b[,6], odds_table2b[,7], odds_table2b$Transformation, odds_table2b$OddsTransformation)
odds3 = get_95_CI(odds_table2b[,9], odds_table2b[,10], odds_table2b$Transformation, odds_table2b$OddsTransformation)
odds4 = get_95_CI(odds_table2b[,12], odds_table2b[,13], odds_table2b$Transformation, odds_table2b$OddsTransformation)
odds5 = get_95_CI(odds_table2b[,15], odds_table2b[,16], odds_table2b$Transformation, odds_table2b$OddsTransformation)
odds6 = get_95_CI(odds_table2b[,18], odds_table2b[,19], odds_table2b$Transformation, odds_table2b$OddsTransformation)

odds_tab = odds_table2b
...
odds[1,2] <- cell_spec(odds_tab[1,2] , bold = T)
odds[2,3] <- cell_spec(odds_tab[2,3], bold = T)

odds[1,4] <- cell_spec(odds_tab[1,4] , bold = T)
odds[2,4] <- cell_spec(odds_tab[2,4] , bold = T)
odds[3,4] <- cell_spec(odds_tab[3,4] , bold = T)
odds[4,4] <- cell_spec(odds_tab[4,4] , bold = T)

odds[1,5] <- cell_spec(odds_tab[1,5] ,  bold = T)
odds[2,5] <- cell_spec(odds_tab[2,5] ,  bold = T)
odds[3,5] <- cell_spec(odds_tab[3,5] ,  bold = T)
odds[4,5] <- cell_spec(odds_tab[4,5] ,  bold = T)

odds[2,6] <- cell_spec(odds_tab[2,6], bold = T)
odds[2,7] <- cell_spec(odds_tab[2,7], bold = T)
# 
```

```{r}
odds_short = odds[,c(1,2,4,5)]

odds_short[1,1] = "Mass % Change *"
odds_short[4,1] = "Wing-to-Body *"
```

```{r}
odds_s = odds_short
odds_s[3,1] <- cell_spec(odds_short[3,1] , extra_css="padding-left:30px")

rownames(odds_s) = NULL
colnames(odds_s) = c("Variables in the Model", "Flew twice rather than flew in T1", "Flew twice rather than did not fly", "Flew in T1 rather than did not fly")
```
