...
# doesn't work b/c it's html:
# save_kable(paper_table, file = paste0(outpath,"paper-table1-latex.tex"), keep_tex = TRUE)

# OR can work with latex only to create the table and save that:
...
```{r echo=FALSE}
colnames(table2b) = c("host plant", "population", "site", "latitude", "longitude", "$\ n_{site}$", "$\ n_{pop}$")
top_caption = "Table 1. Field soapberry bugs collected and tested for flight trials."
kable(table2b) %>% # caption=top_caption
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))  %>%
  add_header_above(c(" ", " ", " ", "Coordinates" = 2, "Sample Sizes" = 2 )) %>% 
...
```{r}
colnames(table2b) = c("host plant", "population", "site", "latitude", "longitude", "n (site)", "n (pop)")

appendix_table = kable(table2b, table.attr = "style='width:66%;'") %>% # caption=top_caption
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))  %>%
  add_header_above(c(" ", " ", " ", "Coordinates" = 2, "Sample Sizes" = 2 )) %>% 
  add_header_above(c("Winter 2020" = 7 )) %>%
  column_spec(1, width="3cm", background="white") %>%
  kable_classic(html_font = "Cambria") %>%
  row_spec(seq(1,nrow(table2b),2), background = "#f5f5f5")

outpath = "/Users/anastasiabernat/Desktop/git_repositories/SBB-dispersal/avbernat/Dispersal/Winter_2020/stats/tables/"
# save_kable(appendix_table, file = paste0(outpath,"appendix_A-table1-latex.png"), keep_tex = TRUE, zoom = 5)
```
...
num_bugs_FAll = nrow(Fall_unique)
```

```{r echo=FALSE}
pop_sample_sizes <- c()
for (pop in pops) {
  n <- nrow(Fall_unique[Fall_unique$population == pop,])
  pop_sample_sizes <- c(pop_sample_sizes, n)
}
...
site_sample_sizes <- c()
sites <- unique(Fall_unique$site)
for (site in sites) {
  n <- nrow(Fall_unique[Fall_unique$site == site,])
  site_sample_sizes <- c(site_sample_sizes, n)
}
...
coor_df <- unique(Fall_unique[,3:7]) %>%
  arrange(desc(latitude), population) 

main <- merge(coor_df , site_df, by=c("site"))
table2a <- merge(main , pop_df,by=c("population"))
table2a <- table2a[, c(3,1,2,4,5,6,7)]
table2a <- table2a[order(-table2a$latitude),] # 25.12846, -80.40809 need to rewrite for JP Grove
```

```{r echo=FALSE}
write.table(table2a, "tables/table2.csv", sep=",", row.names=FALSE)

rownames(table2a) <- NULL
colnames(table2a) = c("host plant", "population", "site", "latitude", "longitude", "$\ n_{site}$", "$\ n_{pop}$")
```

```{r echo=FALSE}
colnames(table2a) = c("host plant", "population", "site", "latitude", "longitude", "$\ n_{site}$", "$\ n_{pop}$")
kable(table2a, caption="Table 1. Field soapberry bugs collected and tested for flight trials.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))  %>%
  add_header_above(c(" ", " ", " ", "Coordinates" = 2, "Sample Sizes" = 2 )) %>% 
...
```{r}
colnames(table2a) = c("host plant", "population", "site", "latitude", "longitude", "n (site)", "n (pop)")

top_caption = "Table 1. Field soapberry bugs collected and tested for flight trials."
appendix_table =  kable(table2a, table.attr = "style='width:66%;'") %>% # caption=top_caption
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))  %>%
  add_header_above(c(" ", " ", " ", "Coordinates" = 2, "Sample Sizes" = 2 )) %>% 
  add_header_above(c("Fall 2019" = 7 )) %>%
  column_spec(1, width="3cm", background="white") %>%
  kable_classic(html_font = "Cambria")  %>%
  row_spec(seq(1,nrow(table2a),2), background = "#f5f5f5")

outpath = "/Users/anastasiabernat/Desktop/git_repositories/SBB-dispersal/avbernat/Dispersal/Winter_2020/stats/tables/"
# save_kable(appendix_table, file = paste0(outpath,"appendix_A-table2-latex.png"), keep_tex = TRUE, zoom = 5)
```
