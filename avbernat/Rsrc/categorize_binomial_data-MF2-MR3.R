...
categorize_data_MF_MR <- function(data, c_exp_var, b_exp_var1, response_var, bin_interval, interval, initial_val) {
  
  df<- data[,c(c_exp_var,response_var, b_exp_var1)]
  df <- df[order(df[c_exp_var]),]
  
  bins <- round((max(df[c_exp_var]) + abs(min(df[c_exp_var]))) / bin_interval)
