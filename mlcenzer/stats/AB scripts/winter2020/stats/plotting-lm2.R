...
lm_plot <- function(d, explanatory_var, response_var, xl, yl, title) {
  ...
  plot(x, y, xlab=xl, ylab=yl, main=title, cex.main=0.95, xaxt="n") # rangi2
  abline(coef[1], coef[2], col="blue")
  #mtext(eq, side=4, cex=0.85)
  shade(mu.PI, x.seq, col=col.alpha("blue"))
  text(mean(x) + 0.2*sd(x), mean(y) + 1.6*sd(y), p, col= color, cex=0.9)
  text(mean(x) + 0.2*sd(x), mean(y) + 1.3*sd(y), corr, col= color, cex=0.9)
  text(mean(x) + 0.2*sd(x), mean(y) + 1.9*sd(y), eq, col= color, cex=0.9)
  #text(400, 15000, p, col= color, cex=0.9)
